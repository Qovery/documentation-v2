...
<Check>You have created a [Project](/configuration/project)</Check>
<Check>You have created an [Environment](/configuration/environment)</Check>
<Check>You have a Git repository or [container image](/configuration/organization/container-registry) to deploy</Check>
...
<Note>
Container registry credentials must be configured at the organization level. See [Container Registry Integration](/configuration/organization/container-registry).
</Note>
...
Configure GPU resources for AI/ML workloads, scientific computing, or graphics-intensive applications.

**Requirements**:
- GPU-enabled cluster (AWS with Karpenter)
- GPU instance types configured on your cluster

**Configuration**:
- **GPU**: Number of GPUs assigned to each instance (default: 0)

<Info>
The GPU feature must be enabled on your cluster to use GPU instances. See [AWS with Karpenter setup](/integrations/kubernetes/eks/managed) for details on configuring GPU instance types.
</Info>

**Example Use Cases**:
- **Machine Learning Training**: TensorFlow, PyTorch workloads
- **Inference Workloads**: Real-time ML predictions
- **Video Processing**: Transcoding, rendering
- **Scientific Computing**: CUDA applications
...
resources:
  cpu: 4000m
  memory: 16 GB
  gpu: 1  # Allocate 1 GPU
```

<Warning>
GPU resources are expensive. Ensure your application actually utilizes the GPU to justify the cost. Monitor GPU utilization in your application metrics.
</Warning>

<Tip>
Start with `gpu: 0` and only add GPU resources when you've confirmed your application needs them. Many ML inference workloads perform well on CPU-only instances.
</Tip>
...
**How It Works**:
- Auto-scaling triggers when CPU consumption exceeds 60% for 15 seconds
- Additional instances are added automatically
- Downscaling occurs if CPU stays below 60% for at least 5 minutes
- Kubernetes [metrics-server](https://github.com/kubernetes-sigs/metrics-server) monitors resource usage
...
<Info>
GPU-enabled instances do not auto-scale. Each GPU instance runs with the configured GPU allocation.
</Info>
...
<Note>
**Important**: Your application should listen on `0.0.0.0:$PORT` rather than `127.0.0.1` or `localhost`. While most things work with localhost, some frameworks (like Node.js) may have issues.
</Note>
...
- **Timeouts**: Request/connection timeouts (default: 60 seconds for public connections)
- **Rate Limiting**: Requests per second limits
- **IP Whitelist**: Restrict access by IP
- **Basic Auth**: Add authentication layer

<Warning>
**Connection Timeouts**: Public port connections are automatically closed after 60 seconds by default. For long-lived connections (like WebSockets), configure custom ingress timeouts in [Advanced Settings](/configuration/service-advanced-settings#network-settings).
</Warning>

<Info>
**TCP/UDP Ports**: Exposing TCP/UDP ports publicly requires a dedicated load balancer, which takes approximately 15 minutes to provision and incurs additional cloud provider costs.
</Info>
