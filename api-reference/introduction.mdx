...
<Info>
**Token vs Bearer**: Qovery API uses `Token` authentication scheme, not `Bearer`. While both are valid HTTP authorization schemes, Qovery specifically expects `Authorization: Token <your-token>` format. Using `Bearer` will result in authentication failure.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Code Examples" icon="code" href="/api/examples">
...
- **Discord Community**: [Join our Discord](https://discord.qovery.com)
