...
Qovery needs AWS credentials to provision and manage resources in your AWS account. You can choose between two methods:

<Warning>
**Security Recommendation**: We strongly recommend using **STS Assume Role** for production environments. Static credentials are long-lived and more exposed to leaks, while Assume Role provides short-lived, automatically refreshed credentials with granular access control.
</Warning>

<Tabs>
  <Tab title="STS Assume Role (Recommended)">
    ### Using STS Assume Role via CloudFormation

    This is the most secure method as it uses temporary credentials that are automatically rotated.

    <Steps>
      <Step title="Deploy CloudFormation Stack">
        In the Qovery console during cluster creation, click **Add Credentials** and select **STS Assume Role**.

        Qovery will provide a CloudFormation stack URL. Click it to open AWS CloudFormation in a new tab.
      </Step>

      <Step title="Create Stack in AWS">
        In the AWS CloudFormation console:

        1. The template URL will be pre-filled
        2. Click **Next**
        3. Stack name: `qovery-assume-role` (or your preferred name)
      ...
      <Step title="Wait for Stack Creation">
        Wait 1-2 minutes for the stack to complete. The status should show **CREATE_COMPLETE**.
      </Step>

      <Step title="Copy Role ARN">
        1. Go to the **Outputs** tab of your CloudFormation stack
        2. Copy the **RoleArn** value (format: `arn:aws:iam::ACCOUNT_ID:role/qovery-role`)
      </Step>

      <Step title="Add Role ARN to Qovery">
        Back in the Qovery console:

        1. Paste the **Role ARN** in the credentials form
        2. Provide a **Credentials Name** (e.g., `aws-production`)
        3. Click **Save**
      </Step>
    ...
    **What the CloudFormation stack creates:**
    - IAM Role with trust relationship to Qovery's AWS account
    - Necessary IAM policies for cluster management
    - Secure cross-account access without storing static credentials

  </Tab>

  <Tab title="Static Credentials (Access Key)">
    ### Using IAM User Access Keys

    <Warning>
    This method uses long-lived credentials. Only use for development/testing or if STS Assume Role is not available.
    </Warning>
    ...
    <Tip>
    Regularly rotate your Access Keys (every 90 days recommended) and never commit them to version control.
    </Tip>
  </Tab>
</Tabs>
