...
<Steps>
  <Step title="Access Resource Metrics">
    Navigate to your application → **Metrics** tab
    ...
    **Review Key Metrics**:
    ...
    CPU Usage:
      - Average: 35%
      - Peak: 75%
      - Allocated: 1000m (1 vCPU)

    Memory Usage:
      - Average: 450 MB
      - Peak: 680 MB
      - Allocated: 1024 MB (1 GB)
  ...
  <Step title="Identify Over-Provisioning">
    Look for resources with consistently low utilization:

    **Over-Provisioned Indicators**:
    - CPU usage consistently below 30%
    - Memory usage consistently below 50%
    - No scaling events in the past 30 days
    - Stable traffic patterns
  </Step>

  <Step title="Identify Under-Provisioning">
  ...
  <Step title="Review Cost Data">
    Go to **Organization** → **Cost Management** to view:

    ```yaml
    Monthly Costs:
      - Cluster Infrastructure: $2,450
      - Applications: $1,200
      - Databases: $850
      - Data Transfer: $180
    ...
    Top Cost Drivers:
      1. Production Cluster (AWS): $1,800
      2. PostgreSQL RDS (db.r5.xlarge): $560
      3. Backend API (15 instances): $420
    ```
  </Step>
</Steps>
