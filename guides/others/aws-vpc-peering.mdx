---
title: "AWS VPC Peering with Qovery"
description: "How to peer a Qovery VPC with an existing VPC on AWS"
---

## Overview

While Qovery is all you need to deploy and run your applications on AWS, you might have existing resources in another VPC that you want to access from your Qovery applications. This guide will show you how to set up VPC peering between the Qovery VPC and an existing one in your AWS account.
...
- You have a Qovery cluster deployed on your AWS account
...
Connect an existing VPC on AWS with the VPC of a Qovery managed cluster, enabling deployment of applications that use resources in the existing VPC (such as a PostgreSQL RDS instance, S3 buckets, or other AWS services).
...
<Steps>
  ...
  <Step title="Identify Your VPCs">
    Locate both your existing VPC and the Qovery-managed VPC:
    - **Existing VPC**: Your pre-existing VPC containing resources
    - **Qovery VPC**: Look for VPC with Qovery cluster name in tags
  </Step>
