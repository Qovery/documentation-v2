...
<CardGroup cols={2}>
  ...
  <Card title="HTTP Errors" icon="triangle-exclamation">
    Detect elevated error rates (4xx/5xx responses)
  </Card>
...
<CardGroup cols={2}>
  <Card title="Above (>)" icon="greater-than">
    ...
    **Common use**: CPU usage, memory usage, error rates, latency
...
<Tabs>
  ...
  <Tab title="HTTP Errors">
    ...
    **Query example:**
    ```
    SEND A NOTIFICATION WHEN THE AVERAGE OF ERROR RATE (5XX)
    FOR [SERVICE_NAME] IS ABOVE 5 %
    DURING THE LAST 3 MINUTES
    ```

    **Why**: Average error rate shows overall service health, 5% indicates significant issues, 3 minutes for faster detection.
  </Tab>
</Tabs>
