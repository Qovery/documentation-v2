...
<Steps>
  <Step title="Create Helm Service">
    ...
    <Note>
    If you prefer not to enable cluster-wide resources, see [Step 3](#step-3-configure-helm-chart-settings) for an alternative approach using values override.
    </Note>
...
<Steps>
  ...
  <Step title="Add optional variables">
    ...
    | `CLUSTER_NAME` | `my-cluster-name` or alias of `QOVERY_KUBERNETES_CLUSTER_NAME` | Service | ❌ | Friendly name for your cluster |
...
<AccordionGroup>
  ...
  <Accordion title="Pods not scheduling — maximum pods per node reached">
    **Problem**: Datadog agent pods fail to start or stay in `Pending` state, causing the deployment to time out. Node events show scheduling errors or IP address timeout messages.

    **Cause**: Since Datadog deploys a DaemonSet (at least one agent pod per node), if even a single node has reached its maximum pod capacity, the deployment will fail. This is common on clusters with many services or smaller instance types.
