...
**An unmatched `id=` is not an error.** If no variable matches the mount `id`, the build still runs and the step sees an empty secret. Add `required=true` to the mount to fail the build instead:
...
Build-time variables apply to every service Qovery builds from a Dockerfile: applications, cronjobs, lifecycle jobs, Terraform services, and agentic workflows using a Docker fragment. Services deployed from an existing container image have no build step and are unaffected.
