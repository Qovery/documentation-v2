...
**Description:** Set the name of an environment variable to use as a basic authentication (`login:crypted_password`) from `htpasswd` command. The environment variable must contain the base64-encoded htpasswd credentials. You can generate them with `htpasswd -nb login password | base64`. Multiple entries can be separated by commas.
