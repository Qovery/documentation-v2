...
**Description:** HPA CPU average utilization threshold assigned to the Envoy Gateway deployment serving traffic, expressed as a value between 0.0 and 1.0 (e.g., 0.8 for 80%). When CPU usage exceeds this threshold, Kubernetes will automatically scale up the number of Envoy Gateway replicas to handle increased load. Learn more in the [Envoy Gateway HPA documentation](https://gateway.envoyproxy.io/docs/tasks/operations/customize-envoyproxy/#customize-envoyproxy-horizontal-pod-autoscaler).
...
**Description:** HPA memory average utilization threshold assigned to the Envoy Gateway deployment serving traffic, expressed as a value between 0.0 and 1.0 (e.g., 0.8 for 80%). When memory usage exceeds this threshold, Kubernetes will automatically scale up the number of Envoy Gateway replicas to handle increased load. Learn more in the [Envoy Gateway HPA documentation](https://gateway.envoyproxy.io/docs/tasks/operations/customize-envoyproxy/#customize-envoyproxy-horizontal-pod-autoscaler).
...
**Description:** Minimum number of Envoy Gateway replicas serving traffic for horizontal pod autoscaling. This ensures a baseline level of availability and load distribution for incoming traffic. Must be less than or equal to `envoy.gateway.hpa.max_number_instances`. Learn more in the [Envoy Gateway HPA documentation](https://gateway.envoyproxy.io/docs/tasks/operations/customize-envoyproxy/#customize-envoyproxy-horizontal-pod-autoscaler).
...
**Description:** Maximum number of Envoy Gateway replicas serving traffic for horizontal pod autoscaling. This sets an upper limit on scaling to control resource consumption. Must be greater than or equal to `envoy.gateway.hpa.min_number_instances`. Learn more in the [Envoy Gateway HPA documentation](https://gateway.envoyproxy.io/docs/tasks/operations/customize-envoyproxy/#customize-envoyproxy-horizontal-pod-autoscaler).
...
**Description:** Number of trusted hops in the X-Forwarded-For header for client IP detection. This setting determines how many proxy hops to trust when extracting the real client IP address from the X-Forwarded-For header. See [Envoy Gateway documentation](https://gateway.envoyproxy.io/latest/tasks/traffic/client-traffic-policy/#configure-client-ip-detection) for more details.
...
**Example:** If a request arrives with `X-Forwarded-For: client-ip, proxy1-ip, proxy2-ip` and you set `number_trusted_hops: 2`, Envoy will trust the last 2 hops (proxy1 and proxy2) and identify `client-ip` as the real client.
