...
**Description:** Custom log format for Envoy Gateway access logs. Allows you to define the structure and content of log entries using Envoy's log formatting syntax. See [Envoy Gateway access logging documentation](https://gateway.envoyproxy.io/latest/tasks/observability/proxy-accesslog/#default-access-log) for available format variables and configuration options.
...
**Default Value:** `null` (uses Envoy default log format)

**Example:**
```json
"[%START_TIME%] %REQ(:METHOD)% %REQ(X-ENVOY-ORIGINAL-PATH?:PATH)% %PROTOCOL% %RESPONSE_CODE% %DURATION% %RESP(X-REQUEST-ID)% %REQ(X-FORWARDED-FOR)%"
```
...
**Description:** List of HTTP status codes for which Envoy Gateway should serve custom error pages. Each code must be a valid HTTP status code between 100 and 599.

**Valid values:** HTTP status codes from `100` to `599`

**Default Value:** `null`

**Example:** `[404, 503, 502, 500]`
...
**Description:** Enables HTTP response compression in Envoy Gateway. When enabled, Envoy compresses responses to reduce bandwidth usage and improve performance.
