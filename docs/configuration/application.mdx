    ...
    <Tabs>
      <Tab title="Model A: Shared IAM Role">
        ...
        **Configuration**:
        ...
        3. Configure the trust relationship to allow KEDA's service account to assume the application's role
        4. Configure both scaler and trigger authentication YAML
        ...
        **Parameters**:
        - **Scaler YAML**:
          - `metadata.queueURL`: Full SQS queue URL
          - `metadata.queueLength`: Target messages per pod (scale when exceeded)
          - `metadata.awsRegion`: AWS region of the queue
        - **Trigger Authentication YAML**:
          - `podIdentity.provider: aws`: Use AWS pod identity
          - `podIdentity.identityOwner: workload`: KEDA inherits the IAM role from application's service account

        <Info>
        With `identityOwner: workload`, KEDA automatically inherits the IAM role from your application's service account. You must configure the trust relationship on the IAM role to allow KEDA's service account to assume it.
        </Info>
        ...
        **Application Configuration**:
        ...
        - Service account configured with IAM role in advanced settings
      ...
      <Tab title="Model B: KEDA-Only IAM Role">
        ...
        **Configuration**:
        ...
        2. Configure the trust relationship to allow KEDA's service account to assume this role
        3. Get the IAM role ARN
        4. Configure both scaler and trigger authentication YAML with the role ARN
        ...
        **Trigger Authentication YAML** (paste in "Trigger Authentication YAML" field):
        ...
        podIdentity:
          provider: aws
          identityOwner: workload
          roleArn: arn:aws:iam::123456789012:role/keda-sqs-scaler-role
        ```
        ...
        **Parameters**:
        ...
        - **Trigger Authentication YAML**:
          - `podIdentity.provider: aws`: Use AWS pod identity
          - `podIdentity.identityOwner: workload`: KEDA uses pod identity
          - `podIdentity.roleArn`: Dedicated IAM role ARN for KEDA

        <Info>
        Specifying `roleArn` in the trigger authentication allows KEDA to use a dedicated IAM role separate from your application, providing better security isolation. You must configure the trust relationship on the IAM role to allow KEDA's service account to assume it.
        </Info>
        ...
        **KEDA IAM Role**:
        - Role ARN: `arn:aws:iam::123456789012:role/keda-notification-queue-scaler`
        - IAM permissions: SQS read-only (GetQueueAttributes, CloudWatch metrics)
        - Specified in trigger authentication YAML
        ...
        **Scaler #1**:
        ...
        - **Trigger Authentication YAML**:
          ...
          podIdentity:
            provider: aws
            identityOwner: workload
            roleArn: arn:aws:iam::123456789012:role/keda-notification-queue-scaler
          ```
        ...
        **Behavior**:
        ...
        - Uses dedicated IAM role ARN for secure, isolated access
      </Tab>
    </Tabs>
      </Tab>
