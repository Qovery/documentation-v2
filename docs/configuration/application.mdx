    ...
    <Tabs>
      ...
      <Tab title="Model B: KEDA-Only IAM Role">
        ...
        **Required permissions**:
        - `sqs:GetQueueAttributes` - Read queue metrics
        - `sqs:GetQueueUrl` - Get queue URL

        **Not required**:
        - `sqs:ReceiveMessage` - KEDA only reads metrics, does not consume messages
        ...
        ```json
        {
          ...
          "Statement": [
            {
              "Sid": "ReadQueueAttributesForScaling",
              "Effect": "Allow",
              "Action": [
        ...
        <Note>
        The application has no AWS permissions in this model. Only KEDA has access to read SQS metrics.
        </Note>
        ...
        Only the KEDA operator ServiceAccount is allowed to assume the IAM role. No application ServiceAccount is included.
        ...
        <Info>
        Replace `<account-id>`, `<region>`, and `<oidc-id>` with your AWS account ID, EKS region, and OIDC provider ID. You can find the OIDC provider URL in your EKS cluster details.
        </Info>
        ...
        **Trigger Authentication YAML** (paste in "Trigger Authentication YAML" field):
        ...
        podIdentity:
          provider: aws
          roleArn: arn:aws:iam::<account-id>:role/<keda-sqs-role-name>
        ```
