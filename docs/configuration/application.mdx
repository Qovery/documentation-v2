...
<Tabs>
  <Tab title="No Scaling">
    ...
    Your application runs with a fixed number of instances (typically 1).

    **Configuration**:
    - Set both minimum and maximum instances to the same value
    ...
    <Tabs>
      <Tab title="Model A: Shared IAM Role">
        ...
        **Configuration**:
        ...
        1. Ensure your application has an IAM role with SQS permissions
        2. Configure the KEDA scaler without trigger authentication
        ...
        **Leave "Trigger Authentication YAML" empty** - KEDA will use the application's IAM role.
        ...
        **Parameters**:
        ...
        <Info>
        **identityOwner: operator** is the key setting that tells KEDA to inherit the IAM role from the application's service account.
        </Info>
      </Tab>
      ...
      <Tab title="Model B: KEDA-Only IAM Role">
        ...
        **How it works**:
        ...
        - IAM role ARN configured for KEDA

        **Configuration**:

        1. Create a dedicated IAM role for KEDA with SQS read permissions
        2. Provide the IAM role ARN in your application configuration
        3. Configure the scaler YAML (no trigger authentication needed)
        ...
        **Scaler YAML** (paste in "Configuration YAML" field):
        ...
        queueLength: "5"
        awsRegion: us-east-1
        awsRoleArn: arn:aws:iam::123456789012:role/keda-sqs-scaler-role
        identityOwner: operator
        ```

        **Leave "Trigger Authentication YAML" empty** - KEDA will use the role ARN specified in the scaler configuration.

        **Parameters**:
        - `queueURL`: Full SQS queue URL
        - `queueLength`: Target messages per pod (scale when exceeded)
        - `awsRegion`: AWS region of the queue
        - `awsRoleArn`: Dedicated IAM role ARN for KEDA
        - `identityOwner: operator`: Tells KEDA to use the specified role

        <Info>
        Using IAM role ARN is more secure than static credentials as it leverages AWS's temporary credential system and doesn't require storing long-lived access keys.
        </Info>
      ...
      <Tab title="Model A: Shared IAM">
        ...
        **Scaler #1**:
        ...
        - **Configuration YAML**:
          ...
          identityOwner: operator
          ```
        - **Trigger Authentication YAML**: _(leave empty)_
      ...
      <Tab title="Model B: KEDA-Only IAM">
        ...
        **Scaler #1**:
        ...
        - **Configuration YAML**:
          ...
          queueLength: "10"
          awsRegion: eu-west-1
          awsRoleArn: arn:aws:iam::123456789012:role/keda-notification-queue-scaler
          identityOwner: operator
          ```
        - **Trigger Authentication YAML**: _(leave empty)_
        ...
        **Behavior**:
        ...
        - Uses IAM role ARN for secure, temporary credential access
      </Tab>
    </Tabs>
      </Tab>
