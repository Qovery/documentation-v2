...
<Steps>
  ...
  <Step title="Create Dockerfile">
    Create a Dockerfile that will run Terraform to deploy the Cloudflare Worker:

    ```dockerfile
    FROM hashicorp/terraform:latest

    # Copy Terraform configuration and worker script
    WORKDIR /workspace/terraform/cloudflare
    COPY cloudflare-worker.tf .
    COPY variables.tf .
    COPY worker.js .
  ...
  <Step title="Build and Push Image">
    ...
    ```bash
    docker build -t your-registry/cloudflare-worker-deployer:latest .
    docker push your-registry/cloudflare-worker-deployer:latest
    ```
  </Step>
  ...
  <Step title="Create Lifecycle Job in Qovery">
    **Via Qovery Console**:
    ...
    3. Configure the job:
       ...
       - **Image**: `your-registry/cloudflare-worker-deployer:latest`
       ...
       - **Environment Variables**:
         ...
         - `DOMAIN`: `example.com`
         - `API_KEY` (secret)
         - `QOVERY_ENVIRONMENT_NAME`: (automatically provided by Qovery)
    ...
    **Via Qovery CLI**:
    ...
    qovery lifecycle create \
      ...
      --env CF_API_TOKEN=secret:cf_token \
...
# Deploy Cloudflare Worker as Edge Gateway
# Step 1: Create Dockerfile
# Create a file named Dockerfile in your terraform/cloudflare directory:
#
# FROM hashicorp/terraform:latest
# WORKDIR /workspace/terraform/cloudflare
# COPY . .
...
# docker build -t your-registry/edge-gateway-deployer:latest .
# docker push your-registry/edge-gateway-deployer:latest
#
# Step 3: Create Lifecycle Job in Qovery Console
# - Name: deploy-edge-gateway
# - Image: your-registry/edge-gateway-deployer:latest
# - Trigger: On Environment Start
# - Environment Variables:
#   * CF_API_TOKEN (secret)
#   * BACKEND_HOST: backend.internal.example.com
#   * QOVERY_ENVIRONMENT_NAME (auto-provided)
```
