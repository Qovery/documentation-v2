...
<Steps>
  ...
  <Step title="Create Dockerfile">
    Create a Dockerfile that will run Terraform to provision the S3 bucket:

    ```dockerfile
    FROM hashicorp/terraform:latest

    # Copy Terraform configuration files
    WORKDIR /workspace/terraform/s3
    COPY s3.tf .
    COPY variables.tf .
  ...
  <Step title="Build and Push Image">
    ...
    ```bash
    docker build -t your-registry/s3-provisioner:latest .
    docker push your-registry/s3-provisioner:latest
    ```
  </Step>
  ...
  <Step title="Create Lifecycle Job in Qovery">
    **Via Qovery Console**:
    ...
    3. Configure the job:
       - **Name**: `provision-s3`
       - **Image**: `your-registry/s3-provisioner:latest`
    ...
    **Via Qovery CLI**:
    ...
    qovery lifecycle create \
      ...
      --env APP_NAME=myapp \
      --env TF_STATE_BUCKET=my-terraform-state
    ```
  </Step>
...
<Tabs>
  ...
  <Tab title="Dockerfile">
    ...
    # Copy Terraform configuration and website build
    WORKDIR /workspace/terraform
    COPY static-website.tf .
    COPY ../build /workspace/build

    # Run Terraform and deploy website
    CMD ["/bin/sh", "-c", "\
      terraform init && \
      terraform apply -auto-approve && \
    ...
    **Create Lifecycle Job in Qovery Console**:
    1. Build and push the Docker image
    2. Create a new Lifecycle Job named `deploy-website`
    3. Set image to your registry image
    4. Set trigger to "On Environment Start"
