...
<Steps>
  ...
  <Step title="Create Dockerfile">
    Create a Dockerfile that will run Terraform to provision BigQuery resources:

    ```dockerfile
    FROM hashicorp/terraform:latest

    # Copy Terraform configuration files
    WORKDIR /workspace/terraform/bigquery
    COPY bigquery.tf .
    COPY variables.tf .
  ...
  <Step title="Build and Push Image">
    ...
    ```bash
    docker build -t your-registry/bigquery-provisioner:latest .
    docker push your-registry/bigquery-provisioner:latest
    ```
  </Step>
  ...
  <Step title="Create Lifecycle Job in Qovery">
    **Via Qovery Console**:
    ...
    3. Configure the job:
       ...
       - **Image**: `your-registry/bigquery-provisioner:latest`
       - **Trigger**: Select "On Environment Start"
       - **Environment Variables**:
         - `GOOGLE_CREDENTIALS` (secret) - JSON service account key
    ...
    **Via Qovery CLI**:
    ...
    qovery lifecycle create \
      ...
      --env GOOGLE_CREDENTIALS=secret:gcp_credentials \
